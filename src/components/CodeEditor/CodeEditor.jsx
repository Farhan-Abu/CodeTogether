...
const CodeEditor = ({ socketRef, roomId, onCodeChange, onStartCall }) => {
    ...
    <div className="codeeditor">
      <div className="header">
        ...
        <div className="effects">
            ...
            <select onChange={(e) => setTheme(e.target.value)} value={theme}>
              {Object.keys(themes).map((themeKey) => (
                <option key={themeKey} value={themeKey}>
                  {themeKey.charAt(0).toUpperCase() + themeKey.slice(1)} ({themes[themeKey].mode})
                </option>
              ))}
            </select>
          </div>
          ...
          <button
            onClick={() => {
              ...
              if (onStartCall) {
                ...
                console.warn("onStartCall is undefined");
              }
            }}
            data-tooltip-id="callbtn"
            data-tooltip-content="Start Call"
          >
            <i className="fa-solid fa-phone"></i>
          </button>
          <Tooltip id="callbtn" style={{ backgroundColor: 'white', color: '#333' }} />
          ...
          <Tooltip id="downloadbtn" style={{ backgroundColor: 'white', color: '#333' }} />
        </div>
      </div>
            ...
            {Array.isArray(tabs) &&
              tabs.map((tab, i) => (
                <div key={tab.id} className={tab.id === isActiveId ? 'tab active' : 'tab'}>
                  <button onClick={() => setIsActiveId(tab.id)}>Tab {i + 1}</button>
